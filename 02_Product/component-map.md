...
- **Header nav** (`app/index.html`, logic in `app/scripts/identity.js`, styles in `app/styles/components.css`) — wordmark, group switcher, crew avatar stack. Built and wired to Supabase — no more mock data.
  ...
  - Group switcher: shows the person's current group, click opens a menu listing all their groups (`getMyGroups`) + "New group" (`createGroup`, inserts a group + membership). Empty state: "No groups yet" when the person has none. "Copy invite link" used to live in this menu — moved out (see the Invite link entry below) since it was ambiguous which group it acted on once someone belonged to more than one.
...
- **Invite link** (`app/index.html`) — a quiet "Invite a friend" text link on the main page (above the Add Show panel, below the show-list header), not in the header at all. Tracks a `currentGroup` object (not just `currentGroupId`) set in `selectGroup`, so the link it builds is always unambiguously scoped to whichever group is actually on screen — the header menu version of this button was removed because "current" wasn't obvious from inside a group-switcher dropdown once someone belonged to more than one group.
...
- **List/Calendar toggle** (`app/index.html`, styles in `app/styles/components.css`) — segmented control (same visual pattern as the show-card response control) above the show list. Both views render from one shared fetch (`loadGroupData` → `renderCurrentView` dispatches to `renderListView` or `renderCalendarView`) so they can't drift out of sync with each other.
  ...
  - **Calendar**: a month grid (single-letter weekday columns on an inverted black/white header row, prev/next navigation, defaults to the real current month). Fixed-height uniform cells (not min-height) so rows never stretch unevenly; shrinks further below the 600px breakpoint so 7 columns stay legible on a phone. Any day with a show gets a pink-tinted background (`.has-shows`) so it reads at a glance before even looking inside the cell. Each show renders as a small pink dot + time (falls back to title text if no time is set) — Google/Apple-calendar style, not full title text, since a cell this size can't hold it. Bold when overlap ≥ 2. Caps at 2 visible entries per day with a plain "+N more" label beyond that (not yet interactive — fine at friend-group scale). Tapping a bubble opens the show detail page.
...
- **Show detail page** (`app/index.html`) — reached via "View details" on a list card or by tapping a calendar bubble; the door calendar cells needed since they're too small to hold a full response control. A plain "‹ Back" link returns to whichever view (list or calendar) was showing before.
  - **Price + Buy tickets**: only on the detail page, not the compact list card. `search-tickets` maps Ticketmaster's `priceRanges` into `price_min`/`price_max`/`price_currency`; `parse-show-link` makes a best-effort attempt at the same from JSON-LD `offers` (not always present). `formatPriceRange()` renders "$45" or "$45–$120", or nothing at all if there's no price data — never a misleading "$0". "Buy tickets" links to `source_url` when present, same URL the title already links to, just a more prominent CTA. Manual entries won't have price data (no field collects it), same as time/openers.
  - Below that: the same title/venue/openers/avatars/response-control block as a list card, built from a shared `renderShowInteractiveBody()` helper used by both surfaces so they can't drift apart.
  - **Listen on Spotify**: a quiet text link next to price/Buy tickets, using `spotify_url` — sourced free from Ticketmaster's own attraction data (`_embedded.attractions[0].externalLinks.spotify`) in `search-tickets`, not a separate Spotify API integration. Worth noting: a real Spotify Web API integration was checked and ruled out — their February 2026 lockdown caps Developer Mode apps at 5 authorized users and removed the endpoints (Top Tracks, Related Artists, Recommendations) that would've been most useful here anyway, so the free Ticketmaster-sourced link is the better option, not just the cheaper one.
  - Considered and cut: an artist-news link — scope creep that doesn't serve the overlap mechanic.
