...
- **Name gate** (`app/index.html`, logic in `app/scripts/identity.js`) — the whole "onboarding" surface, and deliberately not a splash page: when a browser has no `otr_person_id` cookie, the header and main content stay hidden and a centered box shows instead (`.name-gate-center`, flex-centered both axes, `.name-gate-box` capped at 400px — same treatment for every step below). Replaces the old `window.prompt()`.
  - **Two paths, decided once at load** by whether `?g=<slug>` is in the URL (checked before `handleGroupJoinFromUrl` strips it): an invite-link visitor sees "Track shows with your friends" and, after the name step, joins straight in — no further setup needed since the group already exists. A cold-start visitor (no link, no groups) sees "Track shows with friends and set up your first group" and gets walked through two more steps in the same gate: name their first group, then a share nudge ("Copy invite link" / "Continue") right after it's created — the moment an invite link is most worth putting in front of someone, since the group is otherwise empty.
